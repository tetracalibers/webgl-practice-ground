...
export const onload = () => {
  ...
  const uniforms = new UniformLoader(gl, ["uCoef", "uSharpenOn", "uAngle", "uGrayScaleOn"])
  ...
  const defaultCoef = 1.5
  const defaultAngle = 0
  const defaultSharpenOn = false
  ...
  const initGuiControls = () => {
    ...
    ui.number("Emboss Coef", defaultCoef, 0.1, 5.0, 0.01, (value) => {
      uniforms.float("uCoef", value)
    })
    ui.number("Ligth Angle", defaultAngle, 0, 315, 45, (value) => {
      uniforms.int("uAngle", value)
    })
    ui.boolean("Sharpen", defaultSharpenOn, (isActive) => {
      uniforms.boolean("uSharpenOn", isActive)
    })
  ...
  const configure = async () => {
    ...
    uniforms.int("uAngle", defaultAngle)
    uniforms.float("uCoef", defaultCoef)
    uniforms.boolean("uSharpenOn", defaultSharpenOn)
