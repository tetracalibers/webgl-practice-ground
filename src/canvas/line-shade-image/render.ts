...
import imageWater from "@/assets/original/twinkle_00029.jpg"
...
export const onload = () => {
  ...
  const uniforms = new UniformLoader(gl, ["uLineScale", "uLightDirectionZ", "uGrayScaleOn"])

  const images = [
    { name: "金魚", image: imageGoldFish },
    { name: "立方体", image: imageGeometry },
    { name: "紅葉", image: imageAutumnLeaves },
    { name: "森", image: imageWater }
  ]
  const imageNames = images.map((obj) => obj.name)
  let activeImage = 0

  const defaultGrayScaleOn = true
  const defaultLineScale = 1.26
  const defaultLightDirectionZ = 1.0
  ...
  const initGuiControls = () => {
    ...
    ui.number("LineScale", defaultLineScale, 0.6, 2.0, 0.02, (value) => {
      uniforms.float("uLineScale", value)
    })
    ui.number("LightDirection.z", defaultLightDirectionZ, 1.0, 1.5, 0.01, (value) => {
  ...
  const configure = async () => {
    ...
    uniforms.float("uLineScale", defaultLineScale)
